...
// PDI Cecilio -> 14 62 99 13 94
// Bicing Uri -> 212 176 28 10 114
// Metro Adri -> 190 217 73 153 183
                 
int card[][5] = {{32,231,137,171,229}, {212,176,28,10,114},
                 {14,62,99,13,94}, {190,217,73,153,183}};
...
void refund() {
  ...
  if (data.length() != 0 and prod >= 0 and prod <= 5) {

    if (bought[prod] == true) {
      
      if (totalcost == 0) {
        Serial.print("Returning product number ");
        Serial.println(data);
        bought[prod] = false;
        Serial.println("Now it is available again!");
        Serial.println("");
      }
      else {
        Serial.print("Cancelling the purchase of product number ");
        Serial.println(data);
        bought[prod] = false;
        totalcost -= prices[prod];
      ...
      Serial.print("You are trying to return product number ");
      Serial.println(prod);
      Serial.println("But this product has not been bought yet!");
      Serial.println("");
    }
